...
CORE_FILTER_DEFINITIONS_BY_GROUP: dict[str, dict[str, CoreFilterDefinition]] = {
    "events": {
        ...
        "$mcp_auth_failed": {
            ...
            "description": "Fires when an MCP request is refused before a session is established, because the credential was rejected or lacked a required scope. Emitted by PostHog's own MCP server rather than the SDK. Carries `$mcp_auth_failure_reason`, `$mcp_auth_method`, and the client identity, so connectors stuck re-authorizing are visible instead of appearing as an absence of traffic.",
    ...
    "event_properties": {
        ...
        "$mcp_auth_failure_reason": {
            ...
            "description": "Why an MCP request was refused, on $mcp_auth_failed: insufficient_scope (the token is valid but the API denied the call), inactive_oauth_token, invalid_api_key, missing_token, invalid_token_format, or unknown. insufficient_scope also carries $mcp_missing_scope when the API named one.",
