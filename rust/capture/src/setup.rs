...
use crate::config::{AiRouting, AiSinkMode, CaptureMode, Config};
...
pub async fn build_components(
    ...
    // topic, shared by the v0 and v1 analytics pipelines. This stays on the
    // same sink and only changes the destination topic.
...
mod tests {
    ...
    fn parse_token_allowlist_trims_and_drops_empties() {
        // A stray space or trailing/double comma in
        // CAPTURE_ANALYTICS_AI_EVENTS_ALLOWLIST_TOKENS must not produce a
        // mismatched or empty token that breaks routing.
