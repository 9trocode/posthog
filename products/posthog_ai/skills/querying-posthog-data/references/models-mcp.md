...
**Refused requests are a separate event.** A request the PostHog API rejects dies before any session, organization, or project is resolved, so it emits none of the events above — it emits `mcp_auth_failed` instead. That name is non-`$` on purpose: no SDK emits it, so it is PostHog's own instrumentation (a sibling of the `oauth_*` events) rather than part of the customer MCP vocabulary, and it is not registered in `taxonomy.py`. It is also **not** one of the frozen legacy `mcp_*` aliases.

It carries the same `$mcp_*` client-identity fields as the events above, plus its own plain-named fields: `reason` (`insufficient_scope`, `inactive_oauth_token`, `invalid_api_key`, `unknown`), `missing_scope` when the API named a scope, and `response_status` (401/403). It does not set `$mcp_is_error`/`$mcp_error_status`, which mean "a tool call failed" — so it never inflates tool error rates.
