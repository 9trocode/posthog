...
class TestExternalDataSchemaOOMEvent(BaseTest):
    ...
    def _schema(self, name: str, team_id: int | None = None) -> ExternalDataSchema:
        ...
        return ExternalDataSchema.objects.create(team_id=team_id, source=source, name=name)
    ...
    def _oom(
        ...
        *,
        ...
        self_report_age_seconds: float | None = 1.0,
        self_peak_buffer_bytes: int | None = None,
        co_tenant_max_peak_buffer_bytes: int | None = None,
        ...
        event = ExternalDataSchemaOOMEvent.objects.for_team(schema.team_id).create(
            ...
            self_report_age_seconds=self_report_age_seconds,
            self_peak_buffer_bytes=self_peak_buffer_bytes,
            co_tenant_max_peak_buffer_bytes=co_tenant_max_peak_buffer_bytes,
        )
        if age_days:
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                {"self_phase": "merge", "self_peak_buffer_bytes": 1_000, "co_tenant_max_peak_buffer_bytes": 900_000},
                0,
            ),
            (
                "smaller_co_tenant_does_not_exonerate",
                {"self_phase": "merge", "self_peak_buffer_bytes": 900_000, "co_tenant_max_peak_buffer_bytes": 1_000},
                1,
            ),
            # Unknown on either side never exonerates: blame requires evidence, absence of it does not.
            (
                "unknown_own_size_still_counts",
                {"self_phase": "merge", "co_tenant_max_peak_buffer_bytes": 900_000},
                1,
            ),
            ...
            ("stale_extract_phase_still_counts", {"self_phase": "extract", "self_report_age_seconds": 999.0}, 1),
            ("unknown_age_extract_phase_still_counts", {"self_phase": "extract", "self_report_age_seconds": None}, 1),
            (
                "stale_larger_co_tenant_still_counts",
                {
                    "self_phase": "merge",
                    "self_report_age_seconds": 999.0,
                    "self_peak_buffer_bytes": 1_000,
                    "co_tenant_max_peak_buffer_bytes": 900_000,
                },
                1,
            ),
        ]
    ...
    def test_recent_count_screens_bursts_with_one_query_per_cluster_on_a_quiet_fleet(self) -> None:
        # The burst rule must not cost one aggregate per row: a schema can carry dozens of
        # occurrences, and this runs on every detection pass. On a quiet fleet one span-wide
        # aggregate per candidate cluster proves its rows burst-free (a sub-window's distinct count
        # cannot exceed the containing span's). Clustering matters here: occurrences days apart must
        # not merge into one span, or the span's accumulated distinct count would trip the threshold
        # on a quiet fleet and degrade every evaluation to a query per row.
        schema = self._schema("orders")
        for _ in range(6):
            self._oom(schema, self_phase="merge")
        self._oom(schema, self_phase="merge", age_days=5)  # far cluster of its own

        # Bounded, not exact: the team-scoped manager adds constant overhead we don't want to pin.
        # What must hold is that the count doesn't scale with rows — 7 events, two clusters, nowhere
        # near 7 queries.
