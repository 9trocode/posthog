...
class TestNotionSource:
    ...
    @parameterized.expand(
        [
            (
                "5xx_after_internal_retries_exhaust",
                "Notion API error (retryable): status=522, url=https://api.notion.com/v1/comments",
            ),
            (
                "dropped_connection_after_internal_retries_exhaust",
    ...
    def test_retryable_marker_matches_raised_message(self, _name: str, error_message: str) -> None:
        ...
        # (5xx, and dropped connections routed through requests.ConnectionError); matching them
        # here keeps that self-recovering failure out of error tracking as noise instead of being
        # logged as an unclassified exception.
