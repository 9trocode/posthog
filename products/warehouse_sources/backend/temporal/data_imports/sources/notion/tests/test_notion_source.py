...
class TestNotionSource:
    ...
    @parameterized.expand(
        [
            ...
            (
                "ssl_error_after_tenacity_exhausted",
                "HTTPSConnectionPool(host='api.notion.com', port=443): Max retries exceeded with url: "
                "/v1/search (Caused by SSLError(SSLEOFError(8, '[SSL: UNEXPECTED_EOF_WHILE_READING] "
    ...
    def test_retryable_marker_matches_raised_message(self, _name: str, error_message: str) -> None:
        # notion.py's _request lets requests.ConnectionError (which SSLError subclasses) and
        # requests.ReadTimeout through its tenacity retry loop; once that budget exhausts, urllib3
        # wraps them the same way it wraps an exhausted 5xx. Matching both keeps these
