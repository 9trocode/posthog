...
class TestEntityPagination:
    def test_pagination_terminates_and_saves_state_after_each_page(self) -> None:
        session = FakeSession(
            get_responses=[
                FakeResponse(json_body={"partners": [{"partnerId": "1"}], "nextPageToken": "tok-2"}),
                FakeResponse(json_body={"partners": [{"partnerId": "2"}]}),
            ]
        )
        manager = FakeResumeManager()

        batches = _rows("partners", session, manager)

        assert [row["partnerId"] for batch in batches for row in batch] == ["1", "2"]
...
class TestSourceResponse:
    ...
    def test_items_is_lazy(self) -> None:
        ...
        session = FakeSession(get_responses=[FakeResponse(json_body={"partners": [{"partnerId": "1"}]})])
        with mock.patch.object(dv, "display_video_360_session", return_value=session):
