...
async def _ensure_required_parents_synced(
    ...
    for parent_name in required_parents:
        ...
        # A parent that is mid-sync doesn't block the child: `resolve_parent_table_ref` pins the
        # read to the parent's last completed snapshot (Delta time travel), so a concurrent
        # rewrite can't hand the child a torn table.
