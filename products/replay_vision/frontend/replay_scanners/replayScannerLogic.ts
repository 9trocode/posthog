...
export interface CreditLimitState {
    /** The limit in credits, or null when the scanner is uncapped. */
    limit: number | null
    /** The toggle is on but the field is empty, which must block the save. */
    isEmptyButEnabled: boolean
    isOn: boolean
    estimatedMonthly: number | null
    creditsPerObservation: number | null
    isBelowEstimate: boolean
    cannotAffordOneScan: boolean
    /** What to put in the field when the user switches the limit on. */
    seedValue: number
}
...
export const replayScannerLogic = kea<replayScannerLogicType>([
        ...
        scanner: {
            ...
            errors: (scanner: ReplayScanner) => {
                ...
                return {
                    ...
                    credit_limit: Number.isNaN(scanner.credit_limit)
                        ? // The toggle is on with an empty field. Block the save rather than silently
                          // saving as unlimited. See `creditLimitState` for the sentinel's definition.
    ...
    selectors({
        ...
        // `credit_limit` carries a third state beyond a number and null: NaN means "toggle on, field still
        // empty", which must block the save rather than save as unlimited. Decoding it here keeps the form
        // validator and the editor card from drifting apart on what the sentinel means.
        creditLimitState: [
            (s) => [s.scanner, s.scannerEstimate],
            (scanner: ReplayScanner | null, estimate: EstimateResponseApi | null): CreditLimitState => {
                const raw = scanner?.credit_limit
                const isEmptyButEnabled = typeof raw === 'number' && Number.isNaN(raw)
                const limit = typeof raw === 'number' && !Number.isNaN(raw) ? raw : null
                ...
                return {
                    limit,
                    isEmptyButEnabled,
                    isOn: limit !== null || isEmptyButEnabled,
                    estimatedMonthly,
                    ...
                    // or a zero one, NaN leaves the field empty rather than inventing a number.
                    seedValue:
