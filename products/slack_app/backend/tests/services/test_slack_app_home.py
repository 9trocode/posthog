...
class TestRenderHomeView:
    def test_empty_state_renders_buttons_and_no_reset(self):
        ...
        # Personal edit always present; reset hidden when there's no override.
    ...
    # A workspace-keyed model can't say "Opus in project A, Sonnet in project B",
    # so it moved to TeamTasksConfig. The Home tab must not resurrect it as a
    # source, or Slack would again shadow the project default.
    def test_source_is_personal_only(self):
        assert (
            resolve_source(_make_row(runtime_adapter="claude", model="claude-opus-4-7")) == PreferenceSource.personal()
        )

    def test_source_unset_when_the_user_row_has_no_pair(self):
        assert resolve_source(None) == PreferenceSource.unset()
        assert resolve_source(_make_row(reasoning_effort="high")) == PreferenceSource.unset()
...
class TestRenderEditModal:
    def test_callback_id_is_the_personal_modal(self):
...
class TestRetiredWorkspaceModal:
    # A Home tab opened before this shipped can still submit the old workspace
    # modal. That must no-op rather than write a workspace row, or Slack would
    # start shadowing the project default again for that workspace.
    def test_workspace_modal_submission_is_ignored(self, slack_integration, mock_slack_client, flag_on):
        payload = _view_submission_payload(
        ...
        assert handle_app_home_view_submission(payload).status_code == 200
