...
class TestRepairReplayLinkedFlagKeys(BaseTest):
    ...
    def test_repairs_a_stale_key_and_is_idempotent(self) -> None:
        ...
        assert self._run(teams=[self.team])["outcomes"] == {"already_correct": 1}

    def test_dry_run_reports_the_repair_without_writing(self) -> None:
        flag = FeatureFlag.objects.create(team=self.team, created_by=self.user, key="replay-gate-v2")
        self._link_flag(self.team, {"id": flag.id, "key": "replay-gate"})

        report = self._run("--dry-run", teams=[self.team])
    ...
    def test_leaves_links_it_cannot_safely_repair_alone(self, outcome: str) -> None:
        ...
        if outcome == "flag_missing":
            ...
            flag_team = (
                self.team if outcome == "flag_soft_deleted" else Team.objects.create(organization=self.organization)
            )
            flag = FeatureFlag.objects.create(
                team=flag_team,
                created_by=self.user,
                key="replay-gate-v2",
                deleted=outcome == "flag_soft_deleted",
            )
    ...
    def test_repairs_a_sibling_team_linking_another_teams_flag(self) -> None:
        ...
        report = self._run(teams=[sibling_team])
