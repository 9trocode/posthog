...
- Every source class (subclass of `_BaseSource` in `sources/common/base.py`) declares:
  - `supported_versions: tuple[str, ...]` — opaque vendor labels, never parsed by the framework, but **tuple order is a contract: oldest first, newest last**. The upgrade-only repin gate (serializer `validate()`) and the source configuration page's version picker both key "newer" off tuple position, so an out-of-order declaration silently breaks upgrade gating. Always append a new version at the end; never insert before existing entries or reorder them. Default `("v1",)` (`UNVERSIONED_API_VERSION`) for vendors without meaningful versioning.
...
- Repinning is self-serve and **upgrade-only**: the customer moves `ExternalDataSource.api_version` forward from the source's configuration page, or via `PATCH` on the source (also exposed as an MCP tool). It is validated against `supported_versions`, rejects moves to a positionally older entry (a retired pin outside the tuple may move to any supported version — that's escaping a dead version, not downgrading), and cancels in-flight syncs for the schemas that follow the source pin. The support runbook ("Updating a warehouse source to a new vendor API version" in the PostHog/runbooks repo) is now only for bulk or scripted moves — including a genuine rollback, which is deliberately not offered via the API or UI.
