...
- **Hard dependency, enforced twice.** Declare it by overriding `get_required_parent_schemas` on the source (wire it to `required_parents_from_endpoint_configs(ENDPOINTS, schema_name)`; add explicit entries for custom-iterator endpoints) and populate `SourceSchema.requires` in `get_schemas`.
  The schema API then refuses to enable a child unless its parent is already enabled — never auto-enable the parent: parent syncs count toward the customer's billed rows, so turning one on must be their explicit action. `import_data_activity_sync` fails a child run with an actionable error until the parent has `should_sync=True` and `initial_sync_complete=True`; a parent that is merely mid-sync doesn't block the child, because `resolve_parent_table_ref` pins the read to the parent's last completed snapshot via Delta time travel.
