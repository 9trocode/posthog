...
export interface onboardingLogicActions {
    ...
    setSecondaryProductKeys: (keys: ProductKey[]) => {
        keys: ProductKey[]
    }
...
// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface onboardingLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        flowIndex: (flow: OnboardingStepDescriptor[], currentFlowStep: OnboardingStepDescriptor | null) => number
        currentStepKey: (currentFlowStep: OnboardingStepDescriptor | null) => OnboardingStepKey | null
        currentStepProductKey: (
            currentFlowStep: OnboardingStepDescriptor | null,
            productKey: ProductKey | null
        ) => ProductKey | null
        totalOnboardingSteps: (flow: OnboardingStepDescriptor[]) => number
        hasNextStep: (flow: OnboardingStepDescriptor[], flowIndex: number) => boolean
        hasPreviousStep: (flowIndex: number) => boolean
        breadcrumbs: (
            productKey: ProductKey | null,
            currentFlowStep: OnboardingStepDescriptor | null,
            secondaryProductKeys: ProductKey[]
...
export type onboardingLogicType = MakeLogicType<
    onboardingLogicValues,
    onboardingLogicActions,
...
export const onboardingLogic = kea<onboardingLogicType>([
    ...
    selectors({
        ...
        currentFlowStep: [
            (s) => [s.flow, s.stepId],
            (flow: OnboardingStepDescriptor[], stepId: string): OnboardingStepDescriptor | null => {
                if (!flow.length) {
        ...
        flowIndex: [
            ...
            (flow: OnboardingStepDescriptor[], currentFlowStep: OnboardingStepDescriptor | null): number =>
                currentFlowStep ? flow.findIndex((step) => step.id === currentFlowStep.id) : -1,
        ],
        currentStepKey: [
            (s) => [s.currentFlowStep],
            (currentFlowStep: OnboardingStepDescriptor | null): OnboardingStepKey | null =>
                currentFlowStep?.stepKey ?? null,
        ],
        currentStepProductKey: [
            (s) => [s.currentFlowStep, s.productKey],
            (currentFlowStep: OnboardingStepDescriptor | null, productKey: ProductKey | null): ProductKey | null =>
                currentFlowStep?.productKey ?? productKey,
        ],
        totalOnboardingSteps: [(s) => [s.flow], (flow: OnboardingStepDescriptor[]) => flow.length],
        hasNextStep: [
            (s) => [s.flow, s.flowIndex],
            (flow: OnboardingStepDescriptor[], flowIndex: number): boolean =>
                flowIndex >= 0 && flowIndex < flow.length - 1,
        ],
        hasPreviousStep: [(s) => [s.flowIndex], (flowIndex: number): boolean => flowIndex > 0],
        breadcrumbs: [
