...
export type ExperimentMeanMetric = ExperimentMetricBaseProperties &
    ExperimentMetricOutlierHandling & {
         ...
         *  exposure denominator, so the per-value means sum back to the overall mean. Splits are
         *  computed from raw values: with CUPED active they decompose the raw mean, which can differ
         *  from the variance-reduced headline. Only valid for 'total' (count) and 'sum' math, and
         *  cannot be combined with breakdownFilter, winsorization, or threshold. High-cardinality
         *  properties are capped at the top 20 values by total contribution, with the remainder
         *  rolled into an "Other" bucket. */
